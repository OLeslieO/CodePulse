...
| 能力 | 当前状态 | 使用条件或限制 |
| --- | --- | --- |
| 原生顶部菜单栏和详情窗口 | 已实现、已编译 | 352px 半透明银白玻璃弹窗；任务、用量、设置详情保留独立窗口 |
...
- 从源码构建：完整 **Xcode 26 或更新版本**，Swift 6 工具链及 macOS 26 SDK（用于编译原生 Liquid Glass 分支）。运行目标仍为 macOS 13，旧系统自动使用兼容材质。当前验证使用 **Xcode 26.6 / Swift 6.3.3**。
...
也可将应用复制到自己的“应用程序”目录。默认启动后显示菜单栏波形图标，自动尝试连接 Desktop；点击图标打开 Pulse 弹窗。收到实时状态后菜单栏更新任务数。点击齿轮进入设置，点击“查看全部任务”进入详情与本地历史；关闭菜单栏时仍可通过重新打开应用进入详情窗口。默认**不控制键盘 LED、不发出账单查询**。
...
界面采用半透明银白玻璃底色、深灰文字与 `#7DEBFF` 青色灯光点缀；可交互文字和进度使用更深的青色，以适应浅色背景。macOS 26 及以上使用系统原生 **Liquid Glass** 材质与圆形玻璃按钮；macOS 13–15 使用 `NSVisualEffectView` 毛玻璃、细高光边缘和兼容按钮。352px 宽的弹窗按信息优先级组织，点击外部或按 Escape 关闭，观察与 LED 控制继续在后台运行。详情窗口使用系统原生分段控件切换任务、用量与设置，保留原生键盘和辅助功能行为。

- **Pulse 标题**：显示当前筛选范围内的活跃任务数与聚合状态。菜单栏使用波形图标；运行时更明亮，待处理时增加感叹号，空闲时变暗。
- **Processes**：需要处理的任务优先，然后是运行任务和已结束任务。纵向时间线显示名称、项目、结构化状态和本次连续观察时长；最多预览 12 条，列表可滚动，完整列表在详情窗口中。这里的计时从应用确认活动开始，不等于真实任务总耗时，重启应用后重新计时。没有任务时，可点击“检查 Codex 连接”直接进入设置。
- **Balance**：显示已验证余额与单位。macOS 14 及以上余额变化使用数字过渡；macOS 13 直接更新。仅当数据源提供已用额度和总额时显示细进度条；PackyCode 账户余额不会生成虚假的套餐进度或重置日期。查询失败时保留并标记上次数据，在余额旁显示错误摘要，详情中可查看完整错误。
- **Caps Signal**：八个圆点表示 LED 的当前控制模式，快闪节奏使用已保存的翻转间隔。未启用或硬件失败时不显示亮灯；该可视化不是物理 LED 的光学反馈。失败时可直接重试或进入设置查看诊断。
- **底部**：显示实时观察连接状态，以及当前可见任务／余额中最近一次数据更新时间。账单刷新与任务观察仍是两条独立连接。

运行指示器有轻微呼吸效果，环境光随聚合活动状态变化，空闲任务保持静态。系统“减少动态效果”会关闭呼吸、数字及环境光过渡，并将快闪预览改为静态提示；这不会改变实体 LED 的设置。“减少透明度”使用不透明银白底色；“增强对比度”同样使用实色底，强化边界并取消空闲行的淡化。图标按钮提供辅助功能名称与悬停说明，原生按钮支持键盘和 VoiceOver。

动效保留在局部：进度填充使用横向缩放，环境光只改变固定光晕的透明度；循环指示器在视图退出时暂停，关闭弹窗会释放其视图。没有为液态玻璃效果引入网页框架或第三方渲染库。

弹窗和详情窗口共用品牌标题与任务时间线，待处理任务优先展示；详情页还保留来源、更新时间与历史 token 记录。余额页使用大字号等宽数字，LED 则集中在八点硬件指示条中。小屏幕下弹窗内容超过可用高度时可整体滚动；设置页由原生 Form 单独负责滚动。

详细任务、账单诊断与账户设置仍在独立窗口中，保留原有配置、钥匙串与 LED 权限身份。删除本地凭据前显示系统确认框，不会删除供应商账号。新版不更改任务判定、审批策略或账单 API。
